...
<div id="optiontemplate" class="optiond hidden">
	<label><input type="radio" name="option" value="<?php echo $noopts+1;?>" /><span></span></label>
</div>
<div id="options">
<?php
$optionresult = dbQuery('SELECT * FROM option WHERE cid = '.dbMakeSafe($cid).' AND rid = '.dbMakeSafe($rid).' ;');
...
?>	<div class="optiond">
		<label>
			<input type="radio" value="<?php echo $row['oid'];?>" name="option"
				<?php if($row['oid'] == $answer) echo 'checked="checked"';?> />
